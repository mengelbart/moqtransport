...
func TestParseSubscribeRequestMessage(t *testing.T) {
	cases := []struct {
		...
		{
			r: bytes.NewReader(
				append(append([]byte{0x00, 0x00, 0x02, 'n', 's', 0x09}, "trackname"...), 0x00, 0x00),
			),
			expect: nil,
		...
		{
			r: bytes.NewReader(
				append(append([]byte{0x00, 0x00, 0x02, 'n', 's', 0x09}, "trackname"...), 0x01, 0x00),
			),
			expect: &subscribeMessage{
				SubscribeID:    0,
				TrackAlias:     0,
		...
		{
			r: bytes.NewReader(
				append(append([]byte{0x011, 0x012, 0x02, 'n', 's', 0x09}, "trackname"...), 0x01, 0x01, 0x01, 0x01, 'A'),
			),
			expect: &subscribeMessage{
				SubscribeID:    17,
				TrackAlias:     18,
		...
		{
			r: bytes.NewReader(
				append(append([]byte{0x00, 0x00, 0x02, 'n', 's', 0x09}, "trackname"...), 0x04, 0x01, 0x02, 0x03, 0x04, 0x01, 0x01, 0x01, 'A', 0x0a, 0x0b, 0x0c),
			),
			expect: &subscribeMessage{
				SubscribeID:    0,
				TrackAlias:     0,
