...
func TestSubscribeUpdateMessageAppend(t *testing.T) {
	cases := []struct {
	...
	}{
		{
			...
			buf: []byte{},
			expect: []byte{
				byte(subscribeUpdateMessageType), 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
			},
		},
		{
			sum: SubscribeUpdateMessage{
			...
			buf:    []byte{},
			expect: []byte{byte(subscribeUpdateMessageType), 0x01, 0x02, 0x03, 0x04, 0x05, 0x01, 0x01, 0x01, 'A'},
		},
		{
			sum: SubscribeUpdateMessage{
				SubscribeID: 1,
			...
			buf:    []byte{0x0a, 0x0b},
			expect: []byte{0x0a, 0x0b, byte(subscribeUpdateMessageType), 0x01, 0x02, 0x03, 0x04, 0x05, 0x01, 0x01, 0x01, 'A'},
		},
	}
...
func TestParseSubscribeUpdateMessage(t *testing.T) {
	cases := []struct {
		...
		{
			data: []byte{0x01, 0x02, 0x03, 0x04, 0x05, 0x01, 0x01, 0x01, 'P'},
			expect: &SubscribeUpdateMessage{
				SubscribeID: 1,
				StartGroup:  2,
				StartObject: 3,
